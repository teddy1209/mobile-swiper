var mobileSwiper = function (selector) {
    ...
    var touchmoveHandle = function (e) {
        ...
        var touch = e.targetTouches[0],
            ...
            offsetX = x0 - x,
            offsetY = y0 - y; 
        // console.log('move:', x, y, offsetX, offsetY)
        hasmoved || (hasmoved = 1, Math.abs(offsetX) > Math.abs(offsetY) && e.preventDefault()); // 阻止滚动
		if(offsetX <= -50) {
			console.log("向右"); 
	...
	this.init = function(list) {
        this.container = list;
        this.autoplay = list.getAttribute('autoplay');
        ...
        this.item.forEach((li,index) => {
            li.addEventListener('click', () => {
                if (this.queue.indexOf(index) < 3) {
                    let step = this.queue.indexOf(index)
                    while (step) {
                        this.queue.push(this.queue.shift());
                        swap();
                        step -= 1;
                ...
                } else if (this.queue.indexOf(index) > this.queue.length-3) {
                    let step = this.queue.length - this.queue.indexOf(index)
                    while (step) {
                        this.queue.unshift(this.queue.pop()); 
                        swap();
                        step -= 1;
